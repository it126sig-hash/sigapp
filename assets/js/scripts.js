...
function createInputForm(name = null, el = 'input', ty = 'text', cl = [], dt = null) {
...
function displayUploadedFiles(input, listId) {
    ...
    Array.from(input.files).forEach((file, index) => {
        ...
        //input tanggal foto
        const tanggalInput = createInputForm('tgl_' + input.name, 'input', 'date', ['form-control', 'flatpickr-human-friendly', 'ml-1'], new Date().toISOString().split('T')[0])
        ...
        if (listId == "list_prod_foto_konstruksi") {
            const keteranganInput = createInputForm('kategoriPekerjaan_' + input.name, 'select', 'select', ['form-control', 'ml-1', 'kategoriPekerjaan'], list_pekerjaan)
            imgContainer.appendChild(keteranganInput);

            const keteranganInput2 = createInputForm('tugasPekerjaan_' + input.name, 'select', 'select', ['form-control', 'ml-1'])
            ...
            document.getElementById('kategoriPekerjaan_' + input.name).addEventListener('change', (event) => {
...
$("#sertifikat_is_balik_nama, #dt-sertifikat_is_balik_nama").change(function() {
    if (this.value === 'Belum') {
